...
def update_json_file(scraped_data, filename="data-article-news-g.json"):
    ...
    # 步驟 A：讀取舊檔案內容（純 JSON 格式：{"focusData": [...], "data": [...]}）
    if os.path.exists(filename):
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                old_obj = json.load(f)
            # 兼容兩種情況：新格式 (dict) 或 舊格式直接係 list
            if isinstance(old_obj, dict):
                existing_data = old_obj.get("data", [])
            elif isinstance(old_obj, list):
                existing_data = old_obj
    ...
    # 步驟 E：組合成一個純 JSON object（前端 fetch 後用 json.focusData / json.data）
    output_obj = {
        "focusData": focus_data,
        "data": updated_data
    }
    
    # 步驟 F：寫入檔案覆蓋舊內容（純 JSON，不再有 var 包裝）
